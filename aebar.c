...
void
fill_gradient (xcb_drawable_t d, int x, int y, int width, int height,
...
void
fill_rect (xcb_drawable_t d, xcb_gcontext_t _gc, int x, int y, int width,
           int height)
{
  xcb_poly_fill_rectangle (
      c, d, _gc, 1, (const xcb_rectangle_t[]){ { x, y, width, height } });
}
...
shift (monitor_t *mon, int x, int align, int ch_width)
{
  switch (align)
    ...
    case ALIGN_C:
      ...
      break;
    case ALIGN_R:
      xcb_copy_area (c, mon->pixmap, mon->pixmap, gc[GC_DRAW], mon->width - x,
                     0, mon->width - x - ch_width, 0, x, bh);
...
{
  /* can render both at the same time */
  if (attrs & ATTR_OVERL)
          ...
          const int x, const int align, const int button)
{
  ...
  // found the closing and check if its just an escaped one
  for (trail = strchr (++str, ':'); trail && trail[-1] == '\\';
       trail = strchr (trail + 1, ':'))
    ;

  // find the trailing and make sure its within the formatting block also
  ...
  *trail = '\0';
  ...
  // a pointer to the string buffer allocated in the main
  a->cmd = str;
  a->active = true;
  a->align = align;
  a->begin = x;
...
parse (char *text)
{
  ...
  if (!(xft_draw = XftDrawCreate (dpy, cur_mon->pixmap, visual_ptr, colormap)))
    {
      fprintf (stderr, "couldnt create xft drawable\n");
    }
              ...
              switch (*p++)
                ...
                case 'A':
                  ...
                  if (isdigit (*p) && (*p > '0' && *p < '6'))
                    button = *p++ - '0';
                  if (!area_add (p, block_end, &p, cur_mon, pos_x, align,
                                 button))
                    return;
                  break;
                ...
                case 'S':
                  ...
                  if (!(xft_draw = XftDrawCreate (dpy, cur_mon->pixmap,
                                                  visual_ptr, colormap)))
                    {
                      fprintf (stderr, "couldnt create xft drawable\n");
                    }
                ...
                case 'T':
                  if (*p == '-')
                    { // reset automatic font selection
                      font_index = -1;
                      p++;
                      break;
                    }
                  else if (isdigit (*p))
                    {
                      ...
                      // otherwise just fallback to the automatic font
                      // selection
  ...
  if (!xcb_request_check (c, cookie))
    {
      ...
      // copy over the width lut as its part of font_info
...
set_ewmh_atoms (void)
{
  ...
  for (int i = 0; i < atoms; i++)
    atom_cookie[i]
        = xcb_intern_atom (c, 0, strlen (atom_names[i]), atom_names[i]);
...
monitor_create_chain (xcb_rectangle_t *rects, const int num)
{
  ...
  // use the first font height as all the font heights have been set to the
  // biggest of the set
  if (bh < 0 || bh > height)
  ...
  // left is a positive number or zero therefore monitors with zero width are
  // excluded
  width = bw;
  for (i = 0; i < num; i++)
    {
      ...
      if (rects[i].width > left)
        {
          monitor_t *mon = monitor_new (rects[i].x + left, rects[i].y,
  ...
  for (i = 0; i < num; i++)
    {
      ...
      oi_reply = xcb_randr_get_output_info_reply (
          c, xcb_randr_get_output_info (c, outputs[i], XCB_CURRENT_TIME),
          NULL);
      ...
      ci_reply = xcb_randr_get_crtc_info_reply (
          c, xcb_randr_get_crtc_info (c, oi_reply->crtc, XCB_CURRENT_TIME),
          NULL);
      ...
      rects[i] = (xcb_rectangle_t){ ci_reply->x, ci_reply->y, ci_reply->width,
                                    ci_reply->height };
...
get_xinerama_monitors (void)
{
  ...
  xqs_reply = xcb_xinerama_query_screens_reply (
      c, xcb_xinerama_query_screens_unchecked (c), NULL);
...
init (char *wm_name, char *wm_instance)
{
      ...
      if (qe_reply && qe_reply->present)
        {
          ...
          xia_reply = xcb_xinerama_is_active_reply (
              c, xcb_xinerama_is_active (c), NULL);
  ...
  if (!monhead)
    {
      ...
      if (bx + bw > scr->width_in_pixels || by + bh > scr->height_in_pixels)
        {
          fprintf (stderr, "the geometry specified doesnt fit the screen.\n");
          exit (EXIT_FAILURE);
        }

      // if no randr outputs or xinerama screens &  fall back to using whole
      // screen
  ...
  xcb_create_gc (c, gc[GC_DRAW], monhead->pixmap, XCB_GC_FOREGROUND,
                 (const uint32_t[]){ fgc.v });

  gc[GC_CLEAR] = xcb_generate_id (c);
  xcb_create_gc (c, gc[GC_CLEAR], monhead->pixmap, XCB_GC_FOREGROUND,
                 (const uint32_t[]){ bgc.v });

  gc[GC_ATTR] = xcb_generate_id (c);
  xcb_create_gc (c, gc[GC_ATTR], monhead->pixmap, XCB_GC_FOREGROUND,
                 (const uint32_t[]){ ugc.v });
  ...
  for (monitor_t *mon = monhead; mon; mon = mon->next)
    {
      ...
      // make sure that the window really gets in the place its supposed to be
  ...
  if (!XftColorAllocName (dpy, visual_ptr, colormap, color, &sel_fg))
    {
      fprintf (stderr, "couldnt allocate xft font color '%s'\n", color);
    }
  xcb_flush (c);
}
...
main (int argc, char **argv)
{
  ...
  while ((ch = getopt (argc, argv, "hg:bdf:a:pu:B:F:U:n:o:")) != -1)
    {
      switch (ch)
        {
        case 'h':
          printf ("aebar version %s \n", VERSION);
          printf (
              "usage: %s [ -h ] [ -g ] [ -b ] [ -d ] [-f ] [ -a ] [ -p ] [ -n "
              "] [ -u ] [ -B ] [ -F ]\n"
              "\t-g set the bar geometry "
              "{width}x{height}+{xoffset}+{yoffset}\n"
              "\t-b put the bar at the bottom of the screen\n"
              "\t-d force docking (use this if your WM isnt EWMH compliant)\n"
              "\t-f set the font name to use\n"
              "\t-a number of clickable areas available (default is 10)\n"
              "\t-p dont close after the data ends\n"
        ...
        case 'o':
          ...
          break;
        case 'B':
          dbgc = bgc = parse_color (optarg, NULL, (rgba_t)0x00000000U);
          break;
        case 'F':
          dfgc = fgc = parse_color (optarg, NULL, (rgba_t)0xffffffffU);
          break;
        case 'U':
  ...
  if (areas)
    {
      ...
      if (!area_stack.area)
        {
          fprintf (stderr,
                   "Could not allocate enough memory for %d clickable areas, "
  ...
  // the string is strdupd when the command line arguments are parsed
  free (wm_name);
  // the string is strdupd when stripping argv[0]
  free (instance_name);
  ...
  for (;;)
    {
      ...
      if (poll (pollin, 2, -1) > 0)
        {
          ...
          if (pollin[1].revents & POLLIN)
            { // the event comes from the xserver
              while ((ev = xcb_poll_for_event (c)))
                {
                  ...
                  switch (ev->response_type & 0x7F)
                    ...
                    case XCB_BUTTON_PRESS:
                      ...
                      {
                        area_t *area
                            = area_get (press_ev->event, press_ev->detail,
                                        press_ev->event_x);
                        // respond to the click
                        if (area)
                          {
                            if ((write (STDOUT_FILENO, area->cmd,
                                        strlen (area->cmd)))
                                == -1)
                              return EXIT_FAILURE;
      ...
      if (redraw)
        { // copy our temporary pixmap onto the window
          for (monitor_t *mon = monhead; mon; mon = mon->next)
            {
              xcb_copy_area (c, mon->pixmap, mon->window, gc[GC_DRAW], 0, 0, 0,
                             0, mon->width, bh);
            }
        }
