...
area_add (char *str, const char *optend, char **end, monitor_t *mon, const int x, const int align, const int button)
{
  ...
  // find the trailing and make sure its within the formatting block also reject empty commands
...
parse (char *text)
{
  ...
  case 'T':
  if (*p == '-') { // reset automatic font selection
  font_index = -1;
  p++;
  break;
  } 
...
set_ewmh_atoms (void)
{
  ...
  xcb_change_property(c, XCB_PROP_MODE_REPLACE, mon->window, XCB_ATOM_WM_CLASS, XCB_ATOM_STRING, 8, 12, "aebar\0bar");
  }
}
...
main (int argc, char **argv)
{
  ...
  if (pollin[1].revents & POLLIN) { // the event comes from the xserver
